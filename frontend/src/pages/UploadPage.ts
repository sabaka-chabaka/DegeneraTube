...
export function UploadPage(): HTMLElement {
    ...
    <div id="meta-form" style="display:none">
      ...
      <div class="form-group">
        ...
        <textarea class="form-input" id="description" rows="4" placeholder="Describe your video..." style="resize:vertical"></textarea>
      </div>
      <div class="form-group">
        <label class="form-label">Tags</label>
        <input class="form-input" id="tags" type="text" placeholder="gaming, funny, tutorial (comma separated)" />
      </div>
      <div style="display:flex;gap:12px;margin-top:24px">
        <button class="btn btn-primary" id="upload-btn" style="flex:1;justify-content:center;border-radius:10px;padding:12px">
          Upload
        </button>
    ...
    function selectFile(file: File) {
      ...
      <div class="upload-drop-hint">${(file.size / 1024 / 1024).toFixed(1)} MB</div>`;
    ...
    el.querySelector('#upload-btn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            progressLabel.textContent = 'Upload complete! Processing...';
